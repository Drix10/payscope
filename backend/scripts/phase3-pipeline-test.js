...
const job = { jobId: '00000000-0000-4000-8000-000000000003', organizationId: org, type: 'investigate_incident', attemptNumber: 1, createdAt: '2026-08-22T00:00:00.000Z', incidentId, triggerEventId: '00000000-0000-4000-8000-000000000004' };
...
function supervisor(requiresNoActionFallback = false) { return { hypothesis: 'Bounded fixture hypothesis', primaryFailureCategory: 'infrastructure', objectives: ['Classify supplied evidence'], evidencePriorities: [{ fact: 'Fixture evidence exists', whyItMatters: 'It bounds the analysis.' }], subAgents: [], constraints: ['No external action'], noActionCriteria: ['Evidence is insufficient'], estimatedAutoResolvable: !requiresNoActionFallback, requiresNoActionFallback, confidence: 0.9, reasoning: 'Fixture-only reasoning.' }; }
function risk(root = 'gateway_degraded', evidence = 'strong') { return { failureRootCause: root, evidenceStrength: evidence, confidence: 0.9, causalNarrative: 'Fixture evidence supports the selected category.', evidenceConfidenceRationale: 'The fixture provides the required signals.', alternativeHypotheses: [], falsePositiveCostEstimatePaise: 0, missingEvidence: [], chargebackEvidenceReady: false, evidenceItems: ['Tenant-scoped fixture evidence'], recommendedActionCategory: root.startsWith('fraud') ? 'escalate_fraud' : 'auto_resolve_no_action' }; }
function recovery(action = 'auto_resolve_infrastructure') { return { proposedActions: action ? [{ actionType: action, rationale: 'Bounded fixture proposal.', preconditions: ['Policy permits the action'], expectedOutcome: 'A simulated record is stored.', estimatedRecoveryPaise: null, requiresAutonomousExecution: true }] : [], recoveryProbability: 0.1, confidence: 0.9 }; }
...
async function scenario({ enrichment, policy, metrics, output, expect }) {
  ...
  const repository = {
    ...
    policyContext: async () => ({ policy, stats: { autoResolveFraction: 0.2 }, contact: { incidentAttempts: 0, attemptsLast24Hours: 0, attemptsLast7Days: 0, merchantOptedIn: false, customerReferenceAvailable: true } }),
...
(async () => {
  ...
  await scenario({ enrichment: fixtureEnrichment, policy: infrastructurePolicy, metrics: { merchantFailureRate: 0.2, networkFailureRate: 1, customerIncidentCount: 0 }, output: { supervisor: supervisor(), risk: risk(), recovery: recovery() }, expect: calls => {
    assert.equal(calls[0][0], 'persist'); assert.equal(calls[1][0], 'simulate'); assert.equal(calls[0][1][7][0].actionType, 'auto_resolve_infrastructure');
    assert.equal(calls[0][1][4].toolResults.networkFailureRate, 1);
  } });
  await scenario({ enrichment: { ...fixtureEnrichment, crossBorderFlag: true }, policy: infrastructurePolicy, metrics: { merchantFailureRate: 0.2, networkFailureRate: 0.2, customerIncidentCount: 3 }, output: { supervisor: supervisor(), risk: risk('fraud_confirmed', 'strong'), recovery: recovery('flag_for_review') }, expect: calls => {
    assert.equal(calls[0][1][6].outcome, 'auto_no_action'); assert.equal(calls[0][1][6].noActionReason, 'FRAUD_CONFIRMED_HARD_STOP'); assert.equal(calls[0][1][7].length, 0, 'fraud hard stop must persist no proposal');
  } });
  await scenario({ enrichment: null, policy: infrastructurePolicy, metrics: { merchantFailureRate: null, networkFailureRate: null, customerIncidentCount: null }, output: { supervisor: supervisor(true), risk: risk('unknown', 'weak'), recovery: recovery(null) }, expect: calls => {
    assert.equal(calls[0][1][3].requiresNoActionFallback, true); assert.ok(calls[0][1][4].missingEvidence.length >= 3);
  } });
  await scenario({ enrichment: fixtureEnrichment, policy: infrastructurePolicy, metrics: { merchantFailureRate: 0.2, networkFailureRate: 0.2, customerIncidentCount: 0 }, output: { supervisor: { invalid: true }, risk: risk(), recovery: recovery() }, expect: calls => {
    assert.equal(calls.length, 1); assert.equal(calls[0][0], 'failed', 'invalid structured model output must record autonomous no action without persisting a proposal');
  } });
