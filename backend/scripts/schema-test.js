...
const autonomousMigration = fs.readFileSync(path.join(__dirname, '..', 'supabase', 'migrations', '202608230007_autonomous_lifecycle_and_metrics.sql'), 'utf8');
for (const required of ['payscope_autonomously_simulate_pending_proposals', 'simulated_at', 'auto_no_action', 'payscope_contact_attempts', 'pg_advisory_xact_lock']) assert.match(autonomousMigration, new RegExp(required, 'i'));
const investigationIdempotencyMigration = fs.readFileSync(path.join(__dirname, '..', 'supabase', 'migrations', '202608230008_investigation_trigger_idempotency.sql'), 'utf8');
for (const required of ['trigger_event_id', 'payscope_investigations_trigger_event_unique', 'on conflict (organization_id, incident_id, trigger_event_id)', 'PayScope investigation requires a trigger event', 'trigger event is outside the organization', 'payscope_record_investigation_failure']) assert.match(investigationIdempotencyMigration, new RegExp(required.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'i'));
...
for (const required of ['payscope_verify_audit_chain', 'audit integrity is broken', 'payscope_persist_investigation_with_proposals']) assert.match(autonomousMigration, new RegExp(required, 'i'));
const auditCanonicalizationMigration = fs.readFileSync(path.join(__dirname, '..', 'supabase', 'migrations', '202608230005_audit_confidence_canonicalization.sql'), 'utf8');
for (const required of ['normalized_confidence numeric(4,3)', "p_confidence::numeric(4,3)", "'confidence', normalized_confidence"]) assert.match(auditCanonicalizationMigration, new RegExp(required.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'i'));
...
for (const required of ['payscope_dashboard_metrics', 'p_organization_id', 'attributedRecoveries', 'grant execute']) assert.match(dashboardMetricsMigration, new RegExp(required, 'i'));
for (const required of ['9007199254740991', 'proposalsGenerated', 'proposalsSimulated', 'create or replace function']) assert.match(autonomousMigration, new RegExp(required, 'i'));
...
for (const required of ['policy_decision_recorded', 'autonomous_action_simulated', 'autonomous_no_action_recorded', 'payscope_persist_investigation_with_proposals']) assert.match(autonomousMigration, new RegExp(required, 'i'));
