...
(async () => {
  try {
    const { error: insertError } = await client.from('payscope_queue_jobs').insert({ id: jobId, organization_id: organizationId, job_key: `integration-lease:${jobId}`, job_type: 'enrich_event', payload: { jobId, organizationId, type: 'enrich_event', attemptNumber: 1, createdAt: now, eventId }, next_attempt_at: '2099-01-01T00:00:00.000Z' });
    ...
    const [left, right] = await Promise.all([client.rpc('payscope_claim_queue_job', { p_worker_id: `integration-a-${jobId}` }), client.rpc('payscope_claim_queue_job', { p_worker_id: `integration-b-${jobId}` })]);
