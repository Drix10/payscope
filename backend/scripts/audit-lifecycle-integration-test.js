...
if (process.env.PAYSCOPE_RUN_AUDIT_LIFECYCLE_INTEGRATION !== 'true') {
  console.log('Skipped audit-lifecycle integration test (set PAYSCOPE_RUN_AUDIT_LIFECYCLE_INTEGRATION=true for fixture Test Mode Supabase).');
  process.exit(0);
}
...
(async () => {
  ...
  const persisted = await client.rpc('payscope_persist_investigation_with_proposals', {
    ...
    p_policy_decision: { outcome: 'escalate', escalationReason: 'fixture escalation', matchedPolicyId: null, permittedActions: [] },
  ...
  const humanResolution = await client.rpc('payscope_persist_correlation', {
    p_event_id: eventId, p_organization_id: organizationId,
    p_incident: { id: incidentId, risk_tier: 'MEDIUM', status: 'HUMAN_RESOLVED', total_failed_amount_paise: 1_000, recovered_amount_paise: 0, correlated_event_ids: [eventId], opened_at: now, resolved_at: now, updated_at: now },
    p_enqueue_investigation: false, p_job_id: randomUUID(), p_job_payload: null,
  });
  if (humanResolution.error) throw new Error(humanResolution.error.message);
  ...
  for (const type of ['event_received', 'event_enriched', 'incident_opened', 'policy_decision_recorded', 'incident_escalated', 'investigation_completed', 'human_resolution_recorded']) assert.ok(types.has(type), `missing ${type}`);
