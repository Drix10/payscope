...
(async () => {
  ...
  const fraud = evaluatePolicy({ ...incident, riskTier: 'HIGH' }, { ...risk.analysis, failureRootCause: 'fraud_confirmed' }, recovery.plan, [], { autoResolveFraction: 0.2, humanReviewFraction: 0.2 }, { incidentAttempts: 0, attemptsLast24Hours: 0, attemptsLast7Days: 0, merchantOptedIn: false });
  assert.equal(fraud.escalationReason, 'FRAUD_CONFIRMED_HARD_STOP');
  const stoppedOutreach = evaluatePolicy(incident, risk.analysis, { proposedActions: [{ actionType: 'retry_link_sms', rationale: 'Retry safely', estimatedRecoveryPaise: 1000, requiresOperatorApproval: true }], recoveryProbability: 0.2, confidence: 0.8 }, [], { autoResolveFraction: 0.2, humanReviewFraction: 0.2 }, { incidentAttempts: 2, attemptsLast24Hours: 1, attemptsLast7Days: 3, merchantOptedIn: true });
  assert.equal(stoppedOutreach.outcome, 'auto_no_action');
  assert.equal(stoppedOutreach.permittedActions.length, 0);
  assert.throws(() => evaluatePolicy(incident, risk.analysis, recovery.plan, [], { autoResolveFraction: 2, humanReviewFraction: 0.2 }, { incidentAttempts: 0, attemptsLast24Hours: 0, attemptsLast7Days: 0, merchantOptedIn: false }));
