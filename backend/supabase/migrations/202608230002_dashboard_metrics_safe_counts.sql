-- Preserve JavaScript numeric integrity if a future tenant's aggregate count
-- exceeds the exact-integer range. A missing value is safer than a rounded
-- proposal count on an operator surface.
...
language sql security definer set search_path = public, pg_temp as $$
  ...
  select jsonb_build_object(
    ...
    'evaluation', jsonb_build_object(
      'status', 'not_run', 'split', null, 'fixtureSetVersion', null,
      'runAt', null, 'configurationHash', null, 'modelId', null,
      'sampleCount', 0, 'precision', null, 'recall', null, 'f1', null,
