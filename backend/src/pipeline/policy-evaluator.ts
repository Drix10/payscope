...
export type PolicyGate = { name: 'fraud' | 'dispute' | 'auto_resolve_ceiling' | 'human_review_floor' | 'critical_tier' | 'contact_limits' | 'merchant_policy'; result: 'passed' | 'blocked' | 'restricted' | 'skipped'; rationale: string };
export type PolicyDecision = { outcome: 'auto_with_proposals' | 'auto_no_action' | 'escalate'; permittedActions: RecoveryPlan['proposedActions']; escalationReason: string | null; matchedPolicyId: string | null; gates: PolicyGate[] };
...
export function evaluatePolicy(incident: Incident, risk: RiskAnalysis, recovery: RecoveryPlan, policies: MerchantPolicy[], stats: OrgDailyStats, contact: CustomerContactStats): PolicyDecision {
  ...
  if (stats.humanReviewFraction < STOPPING_RULES.MIN_HUMAN_REVIEW_FRACTION_PER_ORG_PER_DAY) return halt(gates, 'human_review_floor', 'HUMAN_REVIEW_FLOOR_NOT_MET');
  gates.push({ name: 'human_review_floor', result: 'passed', rationale: `Daily human-review fraction ${stats.humanReviewFraction.toFixed(3)} meets the 0.100 floor.` });
  ...
  if (!matched && contactPermitted.length) {
    ...
    return { outcome: 'escalate', permittedActions: [], escalationReason: 'NO_POLICY_MATCH', matchedPolicyId: null, gates };
  }
  ...
  return { outcome: permittedActions.length ? 'auto_with_proposals' : 'auto_no_action', permittedActions, escalationReason: null, matchedPolicyId: matched?.id ?? null, gates };
}
...
function halt(gates: PolicyGate[], name: PolicyGate['name'], reason: string): PolicyDecision {
  ...
  for (const later of ['fraud', 'dispute', 'auto_resolve_ceiling', 'human_review_floor', 'critical_tier', 'contact_limits', 'merchant_policy'] as const) {
  ...
  return { outcome: 'escalate', permittedActions: [], escalationReason: reason, matchedPolicyId: null, gates };
}
