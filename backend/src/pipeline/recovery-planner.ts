...
function validateRecoveryPlan(plan: RecoveryPlan, input: RecoveryPlannerInput): void {
  ...
  if (['fraud_confirmed', 'fraud_suspected'].includes(input.riskAnalysis.failureRootCause) && actions.some(action => action !== 'record_risk_signal' && action !== 'flag_for_review')) throw new Error('Recovery planner cannot propose outreach for fraud');
  if (!input.merchantOptedInToRecovery && actions.some(action => ['deliver_recovery_link_email', 'retry_link_whatsapp', 'retry_link_sms', 'hinglish_voice_script'].includes(action))) throw new Error('Recovery planner cannot propose outreach without merchant opt-in');
