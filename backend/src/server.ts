...
import { createRuntimeConfig, RuntimeConfig } from './config/config';
...
export type PayScopeAppOverrides = {
  /** Verification harnesses may inject a repository; production always builds one from env. */
  repository?: MvpRepository;
  /** Verification harnesses may inject a client; the queue worker is never started in that mode. */
  client?: SupabaseClient;
};

export type PayScopeApp = {
  app: Express;
  config: RuntimeConfig | null;
  pipelineEnabled: boolean;
  start: () => Promise<{ server: ReturnType<Express['listen']>; stop: () => Promise<void> }>;
};

type RateBucket = { tokens: number; lastRefillAt: number; lastSeenAt: number };

export function createPayScopeApp(env: NodeJS.ProcessEnv = process.env, overrides: PayScopeAppOverrides = {}): PayScopeApp {
  const app = express();
  ...
  const pipeline = pipelineEnabled ? (() => {
    ...
    const client = overrides.client ?? new SupabaseClient(config.supabaseUrl, config.supabaseServiceRoleKey);
    const repository = overrides.repository ?? new MvpRepository(client);
  ...
  app.use(cors({ origin: (origin, callback) => callback(null, !origin || allowedOrigins.has(origin)), allowedHeaders: ['Content-Type', 'Authorization', 'X-PayScope-API-Key'], methods: ['GET', 'POST', 'OPTIONS'], maxAge: 600 }));
  ...
  // The dashboard router is bound to the single organization configured for
  // this process; every query it serves is organization_id-scoped in the
  // repository. Multi-tenant access requires separate per-tenant processes
  // or a merchant-authenticated control plane with per-merchant keys.
  ...
  async function start(): Promise<{ server: ReturnType<Express['listen']>; stop: () => Promise<void> }> {
    ...
    if (pipeline) {
      ...
      const modelKey = env.MESH_API_KEY?.trim();
      const model = modelKey ? new MeshModelAdapter(modelKey, env.MESH_MODEL?.trim() || undefined, pipeline.config.modelTimeoutMs) : undefined;
  ...
  return { app, config: pipeline?.config ?? null, pipelineEnabled, start };
}
