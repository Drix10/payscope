...
const executionStateLabel: Record<string, string> = {
  ...
  cancelled: "Cancelled",
  pending: "Queued",
  simulated: "Legacy simulation (not executed)",
};
...
const queueViews: Array<{ value: Incident["status"] | "ALL"; title: string }> =
  [
    { value: "ALL", title: "All active" },
    { value: "OPEN", title: "Open" },
    { value: "MONITORING", title: "Monitoring" },
    { value: "DISPUTE_OPENED", title: "Disputes" },
    { value: "RESOLVED", title: "Resolved" },
    { value: "DISMISSED", title: "No action" },
  ];
...
export default function App() {
  ...
  const [queryResult, setQueryResult] = useState<DashboardQueryResult | null>(
    null,
  );
  ...
  const viewCounts: Record<string, number> = useMemo(
    () => ({
      ...
      MONITORING: incidents.filter(
        (i) => i.status === "MONITORING" || i.riskTier === "MONITOR",
      ).length,
      DISPUTE_OPENED: incidents.filter((i) => i.status === "DISPUTE_OPENED")
        .length,
      RESOLVED: incidents.filter(
        (i) =>
          i.status === "RESOLVED" ||
          i.status === "HUMAN_RESOLVED" ||
          i.recoveredAmountPaise > 0,
      ).length,
      DISMISSED: incidents.filter((i) => i.status === "DISMISSED").length,
      ESCALATED: incidents.filter((i) => i.status === "ESCALATED").length,
      HUMAN_RESOLVED: incidents.filter((i) => i.status === "HUMAN_RESOLVED")
        .length,
    }),
    [incidents],
  );
  ...
  const ordered = useMemo(() => {
    ...
    return [...list].sort(
      (a, b) =>
        ...
        riskOrder[a.riskTier] - riskOrder[b.riskTier],
    );
  }, [incidents, filter]);
  ...
  const totalAtRisk = useMemo(
    () =>
      incidents.reduce(
        (sum, incident) => sum + incident.remainingAmountPaise,
        0,
      ),
    [incidents],
  );
        ...
        if (!controller.signal.aborted && mounted.current && !isBackground) {
          const msg =
            reason instanceof Error
              ? reason.message
              : "Unable to load autonomous incident data.";
    ...
    try {
      ...
      if (!controller.signal.aborted && mounted.current)
        setError(
          reason instanceof Error
            ? reason.message
            : "Unable to answer that queue question.",
        );
    } finally {
      if (mounted.current && queryController.current === controller) {
  ...
  return (
    <main className="min-h-screen bg-[#040406] text-neutral-100">
        ...
        <div className="mx-auto flex max-w-[1600px] items-center justify-between gap-3 px-4 py-3 sm:px-6 lg:px-8">
          <div className="flex items-center gap-3">
            <button
              ...
              className="inline-flex items-center gap-1.5 rounded-lg px-2.5 py-2 text-xs font-semibold text-neutral-400 hover:bg-white/[.05] hover:text-white"
            >
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              disabled={loading}
              className="inline-flex items-center gap-1.5 rounded-lg border border-white/10 bg-white/[.04] px-3 py-2 text-xs font-semibold text-neutral-200 hover:bg-white/[.09] disabled:opacity-50"
            >
              <RefreshCw
        ...
        <aside className="border-b border-white/[.08] bg-[#07080d] lg:min-h-[calc(100vh-65px)] lg:border-b-0 lg:border-r">
          <div className="p-5">
            ...
            <p className="mt-1 text-xs leading-5 text-neutral-500">
              Read the AI record for every verified incident.
            </p>
            <div
                ...
                {ordered.map((incident) => (
                  <li key={incident.id}>
                    <button
                      ...
                      aria-current={
                        selectedId === incident.id ? "page" : undefined
                      }
                    ...
                    >
                      <div className="flex justify-between gap-2">
                        <span
                          className={`rounded-full border px-2 py-0.5 text-[10px] font-bold ${riskClass(incident.riskTier)}`}
                        >
                          {label(incident.riskTier)}
                        </span>
                        <span className="text-[10px] text-neutral-400">
                          {stamp(incident.updatedAt).split(",")[0]}
                        </span>
                      </div>
                      <p className="mt-2 text-sm font-semibold text-neutral-100">
                        {incident.status === "DISMISSED"
                          ? "Policy Blocked — No Outreach"
                          : incident.status === "DISPUTE_OPENED"
                            ? "Dispute Open — Outreach Blocked"
                            : incident.status === "RESOLVED"
                              ? "Payment Recovered & Reconciled"
                              : incident.status === "MONITORING"
                                ? "Monitoring Active Telemetry"
                                : "Payment Incident"}
                      </p>
                      <p className="mt-1 text-xs text-neutral-400">
                        {money(incident.remainingAmountPaise)} at risk
                      </p>
                    </button>
                  </li>
                ))}
          ...
          <div className="border-t border-white/[.08] px-5 py-4 text-xs text-neutral-500">
            <span
              className={`mr-2 inline-block h-2 w-2 rounded-full ${health ? "bg-[#00ff87]" : "bg-amber-300"}`}
            />
        ...
        <section className="min-w-0 px-4 py-6 sm:px-6 lg:px-8 lg:py-8">
          <div className="mx-auto max-w-[1180px]">
            {error && (
              <div
                role="alert"
                className="mb-5 flex gap-3 rounded-2xl border border-rose-400/25 bg-rose-400/[.07] p-4 text-sm text-rose-100"
              >
            ...
            <section className="mb-6 flex flex-wrap items-end justify-between gap-4">
              <div>
                <p className="text-[11px] font-bold uppercase tracking-[.18em] text-[#00ff87]">
                  Autonomous execution ledger
                </p>
                <h1 className="mt-2 text-3xl font-semibold tracking-tight text-white sm:text-4xl">
                  What the AI executed
                </h1>
                <p className="mt-2 max-w-2xl text-sm leading-6 text-neutral-400">
                  The AI investigates, the deterministic policy authorizes, the
                  provider command is dispatched, and the verified receipt is
                  reconciled — all recorded here.
                </p>
              </div>
            </section>
            ...
            <section className="mb-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
              <div className="rounded-2xl border border-white/[.08] bg-[#090a0f]/90 p-4 backdrop-blur-xl">
                ...
                <p className="mt-1 text-[11px] text-neutral-500">
                  {incidents.length} active incidents tracked
                </p>
              </div>
              <div className="rounded-2xl border border-[#00ff87]/25 bg-[#00ff87]/[.06] p-4 backdrop-blur-xl">
                ...
                <p className="mt-2 text-2xl font-extrabold text-[#00ff87]">
                  100%
                </p>
                <p className="mt-1 text-[11px] text-[#86f4bd]/80">
                  Automated Razorpay links & SMTP
                </p>
              </div>
              <div className="rounded-2xl border border-white/[.08] bg-[#090a0f]/90 p-4 backdrop-blur-xl">
                <div className="flex items-center justify-between text-neutral-400">
                ...
                <p className="mt-1 text-[11px] text-neutral-500">
                  Supervisor, Risk & Planner active
                </p>
              </div>
              <div className="rounded-2xl border border-white/[.08] bg-[#090a0f]/90 p-4 backdrop-blur-xl">
                <div className="flex items-center justify-between text-neutral-400">
                ...
                <p className="mt-1 text-[11px] text-neutral-500">
                  Deterministic hard stops enforced
                </p>
              </div>
            </section>
          ...
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              <div className="flex flex-wrap gap-2">
                <span
                  className={`rounded-full border px-2.5 py-1 text-[10px] font-bold uppercase tracking-wide ${riskClass(detail.incident.riskTier)}`}
                >
              ...
              <h2 className="mt-4 text-xl font-semibold text-white sm:text-2xl">
                {detail.investigation?.plan?.hypothesis ??
                  (detail.incident.status === "DISMISSED"
                    ? "Autonomous policy evaluated — outreach blocked by safety rules"
                    : detail.incident.status === "DISPUTE_OPENED"
                      ? "Dispute created — automated outreach suspended"
                      : detail.incident.status === "RESOLVED"
                        ? "Payment recovered and reconciled via Razorpay"
                        : "Payment failure signal under multi-agent investigation")}
              </h2>
              <p className="mt-2 text-sm text-neutral-400">
                {detail.events.length} verified signal
                {detail.events.length === 1 ? "" : "s"} ·{" "}
                {money(detail.incident.remainingAmountPaise)} remains at risk
              </p>
            </div>
            <div className="rounded-xl border border-[#00ff87]/20 bg-[#00ff87]/[.06] px-4 py-3 text-right">
              <p className="text-[10px] font-bold uppercase tracking-[.16em] text-[#86f4bd]">
                AI outcome
              </p>
              <p className="mt-1 text-sm font-semibold text-white">
                {outcomeText(detail.incident.status, latest)}
              </p>
            </div>
          </div>
        </div>
...
function ExecutionLedger({
  ...
  execution: import("./types/mvp").ExecutionActionSummary[];
}) {
  return (
    <section className="mt-6 rounded-2xl border border-white/[.08] bg-white/[.015] p-5 sm:p-6">
      <div className="flex items-center justify-between">
        <div>
          <p className="text-sm font-semibold text-white">Execution ledger</p>
          <p className="mt-1 text-xs text-neutral-500">
            Provider commands and their verified receipt state — never recipient
            data or raw payloads.
          </p>
        </div>
        <span className="text-xs text-neutral-500">
          {execution.length} action{execution.length === 1 ? "" : "s"}
        </span>
      </div>
      {execution.length ? (
        <ol className="mt-4 space-y-2">
          {execution.map((action) => (
            <li
              key={action.id}
              className="flex flex-wrap items-center justify-between gap-2 rounded-xl border border-white/[.07] bg-black/15 p-3"
            >
              <div className="flex items-center gap-2">
                <span
                  className={`rounded-full border px-2 py-0.5 text-[10px] font-bold ${executionStateClass[action.state] ?? "border-white/10 text-neutral-300"}`}
                >
              ...
              <div className="flex items-center gap-3 text-[11px] text-neutral-400">
                {action.amountPaise !== null && (
                  <span className="font-semibold text-neutral-200">
                    {money(action.amountPaise, action.currency ?? "INR")}
                  </span>
                )}
                <span>
                  {action.terminalReason
                    ? label(action.terminalReason)
                    : (action.providerObjectId ?? "no provider receipt")}
                </span>
        ...
        <p className="mt-2 text-xs text-neutral-500">
          No provider command has been dispatched for this incident yet.
        </p>
      )}
    </section>
  );
...
function ActualStages({
  ...
  const isBlocked =
    detail.incident.status === "DISPUTE_OPENED" ||
    detail.incident.status === "DISMISSED" ||
    eventTypes.has("execution_command_blocked") ||
    eventTypes.has("autonomous_no_action_recorded");
  const hasInvestigation =
    detail.investigation !== null || eventTypes.has("investigation_completed");
  const hasPolicy =
    Boolean(detail.investigation?.policyDecision) ||
    eventTypes.has("policy_decision_recorded");
  const hasExecution =
    detail.execution.length > 0 ||
    detail.proposals.length > 0 ||
    eventTypes.has("execution_command_queued") ||
    eventTypes.has("execution_receipt_recorded");
  const isResolved =
    detail.incident.status === "RESOLVED" || eventTypes.has("reconciled");

  const passedGates =
    detail.investigation?.policyDecision?.gates.filter(
      (g) => g.result === "passed",
    ).length ?? 0;
  ...
  const steps = [
    ...
    {
      num: 2,
      label: "2. Multi-Agent Analysis",
      status: hasInvestigation ? "complete" : "pending",
      detail: hasInvestigation
        ? "Supervisor + Risk Analyst"
        : "Running LLM Multi-Agent",
    },
    {
      num: 3,
      label: "3. Safety Gates",
      status: isBlocked ? "blocked" : hasPolicy ? "complete" : "pending",
      detail: gateDetail,
    },
    {
      num: 4,
      label: "4. Idempotent Execution",
      status: isBlocked ? "blocked" : hasExecution ? "complete" : "pending",
      detail: isBlocked ? "Outreach Blocked" : "Razorpay Link & Email Sent",
    },
    {
      num: 5,
      label: "5. Receipt & Audit",
      status: isResolved ? "complete" : hasExecution ? "active" : "pending",
      detail: isResolved
        ? "Payment Reconciled"
  ...
  return (
    <div className="border-t border-white/[.08] px-5 py-4 sm:px-7">
      <div className="flex items-center justify-between">
        <p className="text-[10px] font-bold uppercase tracking-[.16em] text-[#00ff87]">
          Autonomous Pipeline Progression
        </p>
        <span className="text-[10px] font-semibold text-neutral-400">
          5-Stage Verified Execution
        </span>
      </div>
        ...
        {steps.map((step) => (
          <li
            ...
            className={`rounded-xl border p-3 transition ${
              step.status === "complete"
                ? "border-[#00ff87]/30 bg-[#00ff87]/[.08] text-white"
                : step.status === "blocked"
                  ? "border-amber-400/30 bg-amber-400/[.08] text-amber-100"
                  : step.status === "active"
                    ? "border-sky-400/30 bg-sky-400/[.08] text-sky-100"
          ...
          >
            <div className="flex items-center justify-between gap-1">
              ...
              {step.status === "complete" && (
                <CircleCheck className="h-3.5 w-3.5 text-[#00ff87]" />
              )}
              {step.status === "blocked" && (
    ...
    <section className="rounded-2xl border border-white/[.09] bg-white/[.018] p-5 sm:p-6">
      <div className="flex justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-xs text-neutral-500">
            Only signals retained by the durable pipeline.
          </p>
        </div>
      ...
      <ol className="mt-5 space-y-3">
        {events.length ? (
          events.map((event) => (
            <li
              key={event.id}
              className="rounded-xl border border-white/[.07] bg-black/15 p-3.5"
            >
              <div className="flex justify-between gap-3">
                <div>
                  ...
                  <p className="mt-1 text-xs text-neutral-500">
                    {stamp(event.event.occurredAt)}
                  </p>
                </div>
                {event.event.amountPaise !== undefined && (
                  <span className="text-xs font-semibold text-neutral-300">
                    {money(event.event.amountPaise)}
                  </span>
                )}
              </div>
              {event.enrichment && (
          ...
          <li className="rounded-xl border border-dashed border-white/[.12] p-4 text-sm text-neutral-500">
            No verified events are available for this record.
          </li>
        )}
      </ol>
    </section>
...
type ExecutionOrProposal =
  | import("./types/mvp").ExecutionActionSummary
  | Proposal;
...
function AiDecision({
  ...
  detail: IncidentDetail;
  proposal: ExecutionOrProposal | undefined;
  integrity: AuditIntegrity | null;
}) {
  ...
  const actionType = proposal
    ? "capability" in proposal
      ? proposal.capability
      : proposal.actionType
    : undefined;
  const actionStatus = proposal
    ? "state" in proposal
      ? proposal.state
      : proposal.status
    : undefined;
  const plannedAction =
    detail.investigation?.recoveryPlan?.proposedActions.find(
      (action) => action.actionType === actionType,
    );
  const simulatedNote =
    proposal &&
    "deliveryResult" in proposal &&
    typeof proposal.deliveryResult?.note === "string"
      ? proposal.deliveryResult.note
      : null;
  const actionRationale = proposal
    ? "content" in proposal && typeof proposal.content?.rationale === "string"
      ? proposal.content.rationale
      : "terminalReason" in proposal && proposal.terminalReason
        ? proposal.terminalReason
    ...
    <section className="rounded-2xl border border-white/[.09] bg-white/[.018] p-5 sm:p-6">
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-xs leading-5 text-neutral-500">
            Bounded investigation, deterministic gates, provider-confirmed
            outcome.
          </p>
        </div>
      ...
      <div className="mt-5 space-y-3">
        ...
        <DecisionRow
          ...
          title={
            proposal
              ? `${label(actionType)} · ${executionStateLabel[actionStatus || ""] ?? label(actionStatus)}`
              : label(detail.incident.status)
          }
          detail={
            proposal
              ? (simulatedNote ??
                plannedAction?.expectedOutcome ??
                String(actionRationale))
              : (policy?.noActionReason ??
                detail.investigation?.recoveryPlan?.noActionReason ??
                "The AI recorded no external action.")
          }
        />
      </div>
      {risk?.alternativeHypotheses.length ? (
        <p className="mt-4 text-xs leading-5 text-neutral-500">
          Alternatives considered: {risk.alternativeHypotheses.join(" · ")}
        </p>
      ) : null}
      <div
        className={`mt-5 flex gap-2 rounded-xl border p-3 text-xs ${integrity?.status === "intact" ? "border-[#00ff87]/20 bg-[#00ff87]/[.06] text-[#b8f8d8]" : "border-amber-300/20 bg-amber-300/[.06] text-amber-100"}`}
      >
        ...
        {integrity?.status === "intact"
          ? `Audit chain intact · ${integrity.entryCount} entries verified`
          : "Audit integrity is unavailable; no autonomous simulation should proceed."}
      </div>
    </section>
  );
}
...
function DecisionRow({
  ...
  return (
    <div className="flex gap-3 rounded-xl border border-white/[.07] bg-black/15 p-3">
      <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg border border-[#00ff87]/20 bg-[#00ff87]/[.07] text-[#86f4bd]">
        {icon}
      </span>
      <div>
...
function AuditTrail({
  ...
  for (const entry of entries) {
    ...
    if (
      last &&
      last.entry.eventType === entry.eventType &&
      last.entry.decision === entry.decision &&
      last.entry.rationale === entry.rationale
    ) {
      last.count += 1;
      last.entry = entry;
    } else {
    ...
    <section className="mt-6 rounded-2xl border border-white/[.08] bg-white/[.015] p-5 sm:p-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-xs text-neutral-500">
            Append-only records of what the pipeline did.
          </p>
        </div>
        <span
          className={`text-xs ${integrity?.status === "intact" ? "text-[#aaffd5]" : "text-amber-100"}`}
        >
          ...
          grouped.map(({ entry, count }) => (
            <li key={entry.id} className="border-l border-[#00ff87]/35 pl-4">
              ...
              <p className="mt-1 text-xs leading-5 text-neutral-400">
                {entry.rationale || entry.decision}
              </p>
            </li>
          ))
        ) : (
          <li className="text-sm text-neutral-500">
            No audit entries were returned for this incident.
          </li>
        )}
      </ol>
    </section>
...
function Insights({
  ...
  return (
    <section className="mt-6 rounded-2xl border border-white/[.08] bg-white/[.015] p-5 sm:p-6">
      ...
      <p className="mt-1 text-xs text-neutral-500">
        Ask about the tenant-scoped incident record. This cannot trigger an
        action.
      </p>
      <div className="mt-4 grid gap-3 sm:grid-cols-3">
        <Metric
          label="At risk"
          value={
            metrics?.operations.totalAtRiskPaise === null || !metrics
              ? "Not available"
              : money(metrics.operations.totalAtRiskPaise)
          }
        />
        <Metric
          ...
          value={
            metrics
              ? String(metrics.operations.actionsDispatched)
              : "Not available"
          }
        />
        <Metric
          ...
          value={
            metrics
              ? String(metrics.operations.confirmedRecoveries)
              : "Not available"
          }
        />
      </div>
      ...
      >
        ...
        <input
          ...
          maxLength={240}
          placeholder="Example: show open high-risk incidents"
          className="min-w-0 flex-1 rounded-xl border border-white/[.12] bg-black/20 px-3 py-2 text-sm text-white placeholder:text-neutral-600"
        />
        <button
          type="submit"
          disabled={loading || !query.trim()}
          className="rounded-xl border border-[#00ff87]/30 px-3 text-xs font-bold text-[#aaffd5] hover:bg-[#00ff87]/10 disabled:opacity-50"
        >
          {loading ? "Checking…" : "Ask"}
        </button>
      </form>
      {result && (
        <div className="mt-3 rounded-xl border border-white/[.08] bg-black/15 p-3 text-xs">
          <p className="font-semibold text-neutral-100">
            {result.interpretation}
          </p>
          <p className="mt-1 text-neutral-400">
            {result.matchedIncidentCount} matching ·{" "}
            {money(result.matchedRemainingAmountPaise)} at risk
          </p>
        </div>
      )}
    </section>
...
function EmptyList() {
  return (
    <div className="p-5 text-center">
      ...
      <p className="mt-1 text-xs leading-5 text-neutral-500">
        Verified events appear after the autonomous worker finishes.
      </p>
    </div>
  );
}
...
function EmptyDetail({ loading }: { loading: boolean }) {
  return (
    <section className="rounded-2xl border border-dashed border-white/[.12] bg-white/[.015] p-10 text-center">
      <FileSearch className="mx-auto h-7 w-7 text-[#00ff87]" />
      <h2 className="mt-4 text-lg font-semibold text-white">
        {loading ? "Loading incident feed" : "No incident selected"}
      </h2>
      <p className="mx-auto mt-2 max-w-md text-sm leading-6 text-neutral-500">
        Select an incident to see its verified signals and autonomous decision
        record.
      </p>
    </section>
  );
