...
export default function App() {
  ...
  useEffect(() => { mounted.current = true; void refresh(); void fetchPolicies(); const interval = window.setInterval(() => { if (!document.hidden) void refresh(true) }, 30_000); const onVisibilityChange = () => { if (!document.hidden) void refresh(true) }; document.addEventListener('visibilitychange', onVisibilityChange); return () => { mounted.current = false; refreshController.current?.abort(); detailController.current?.abort(); window.clearInterval(interval); document.removeEventListener('visibilitychange', onVisibilityChange) } }, [refresh, fetchPolicies])
