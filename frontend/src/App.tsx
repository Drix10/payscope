...
export default function App() {
  ...
  return <main className="min-h-screen bg-[#040406] text-neutral-100">
    ...
    <section className="mx-auto max-w-[1480px] px-5 py-6 sm:px-8"><div className="grid gap-5 lg:grid-cols-[220px_minmax(0,1fr)]"><MvpSidebar activeFilter={statusFilter} onFilter={setStatusFilter} health={health} incidentCount={incidents.length} /><div className="min-w-0"><div className="mb-5 rounded-xl border border-amber-200/20 bg-amber-200/[.05] p-4 text-sm text-amber-50"><ShieldCheck className="mr-2 inline h-4 w-4" /><strong>Test Mode · proposal-only.</strong> Enrichment is labelled by source, communications are simulated when implemented, and no payment action is available here.{health && <span className="ml-2 text-xs text-amber-100">Demo operator · org {health.organizationId.slice(0, 8)}</span>}</div>
...
function MvpSidebar({ activeFilter, onFilter, health, incidentCount }: { activeFilter: Incident['status'] | 'ALL'; onFilter: (status: Incident['status'] | 'ALL') => void; health: MvpHealth | null; incidentCount: number }) { const items: Array<{ id: Incident['status'] | 'ALL'; label: string; description: string }> = [{ id: 'ALL', label: 'Overview', description: 'At-a-glance' }, { id: 'OPEN', label: 'Incidents', description: 'Payment risk' }, { id: 'ESCALATED', label: 'Escalated', description: 'Human review' }, { id: 'RESOLVED', label: 'Resolved', description: 'Closed safely' }]; return <aside className="h-fit lg:sticky lg:top-[84px]"><div className="rounded-2xl border border-white/[.06] bg-[#090a0f]/60 p-2 backdrop-blur-xl"><nav aria-label="Dashboard sections" className="flex gap-1 overflow-x-auto pb-1 lg:block lg:space-y-0.5 lg:overflow-visible">{items.map(item => { const active = activeFilter === item.id; return <button key={item.id} type="button" onClick={() => onFilter(item.id)} aria-current={active ? 'page' : undefined} className={`group flex min-w-[130px] flex-1 items-center gap-2.5 rounded-xl px-3 py-2.5 text-left transition lg:w-full ${active ? 'bg-white text-black' : 'text-neutral-400 hover:bg-white/[.05] hover:text-white'}`}><span className={`h-2 w-2 rounded-full ${active ? 'bg-[#00ff87]' : 'bg-white/20'}`} /><span className="min-w-0 flex-1"><span className="flex items-center justify-between gap-2 text-[12px] font-semibold"><span>{item.label}</span>{item.id === 'OPEN' && incidentCount > 0 && <span className={`rounded-full px-1.5 py-0.5 text-[10px] ${active ? 'bg-black text-white' : 'bg-rose-500/15 text-rose-300'}`}>{incidentCount}</span>}</span><span className={`mt-1 hidden text-[10px] lg:block ${active ? 'text-black/60' : 'text-neutral-500'}`}>{item.description}</span></span></button> })}</nav><div className="mt-3 rounded-xl bg-black/25 p-3"><p className="text-[10px] font-bold uppercase tracking-wide text-neutral-500">Pipeline</p><p className="mt-2 flex items-center gap-2 text-xs font-semibold text-neutral-200"><span className={`h-2 w-2 rounded-full ${health?.pipeline === 'agentic_mvp' ? 'bg-[#00ff87]' : 'bg-amber-300'}`} />{health?.pipeline ?? 'Checking'}</p><p className="mt-1 text-[10px] leading-4 text-neutral-500">{health ? `${health.database} database · ${health.queueWorker} worker` : 'Reading durable health…'}</p></div></div></aside> }
