...
export type AuditEntry = { id: string; organizationId: string; incidentId: string | null; sequenceNumber: number; eventType: string; actorType: 'system' | 'human'; actorId: string; decision: string; rationale: string; confidence: number | null; enrichmentSource: string | null; createdAt: string }
...
export type RiskAnalysis = { failureRootCause: string; evidenceStrength: 'strong' | 'moderate' | 'weak'; confidence: number; falsePositiveCostEstimatePaise: number; missingEvidence: string[]; chargebackEvidenceReady: boolean; evidenceItems: string[]; recommendedActionCategory: string; toolResults: { incidentTimelineEventCount: number; merchantFailureRate: number | null; networkFailureRate: number | null; customerIncidentCount: number | null } }
...
export type DashboardMetrics = {
  operations: { totalAtRiskPaise: number | null; proposalsGenerated: number | null; proposalsApproved: number | null; attributedRecoveries: number | null; recoveredPaise: number | null; recoveryRate: number | null; contactToRecoveryRatio: number | null }
