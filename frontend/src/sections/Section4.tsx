...
export function Section4({
  ...
  if (isMobile) {
    return (
       <MobileShowcasePanel eyebrow="04 / Operator Dashboard" title="One dashboard. Full control." description="Review incidents, inspect evidence-bound investigations, and record human decisions — or let admin policies auto-handle the safe stuff." accentClass="text-[#00ff87]">
      ...
      <div style={{ position: 'absolute', top: '35px', left: '55px', right: '55px', display: 'flex', flexDirection: 'column', zIndex: 10 }}>
        ...
        >
          <div className="inline-flex items-center gap-2.5 rounded-full border border-amber-500/30 bg-amber-500/10 px-4 py-1.5 backdrop-blur-xl shadow-[0_0_20px_rgba(255,183,3,0.2)]">
            ...
            <span className="text-[11px] font-semibold text-neutral-300">Policy-Driven Autonomy</span>
          </div>
        </motion.div>

        <h1 style={{ fontFamily: 'var(--font-jakarta)', fontSize: '46px', fontWeight: 300, color: '#ffffff', margin: 0, marginBottom: '6px' }}>
          <BlurFadeWords text="Human Control. Policy Autonomy." baseDelay={0.4} isInView={isInView} />
        </h1>

        <p style={{ fontFamily: 'var(--font-jakarta)', fontSize: '17px', fontWeight: 300, color: 'rgba(255,255,255,0.7)', margin: 0, marginBottom: '18px' }}>
           <BlurFadeWords text="Set thresholds once — the agent auto-executes safe actions within bounds. Everything else stays human-approved." baseDelay={0.7} isInView={isInView} />
        </p>
        ...
        >
          ...
          <div className="rounded-xl border border-white/10 bg-white/[0.03] p-3 backdrop-blur-md">
              <div className="flex items-center gap-1.5 text-xs font-semibold text-amber-400">
                <ShieldCheck className="h-3.5 w-3.5" /> 3. Human or Policy
             </div>
             <p className="mt-1 text-[11px] text-neutral-400">
                Approve manually, or let thresholds auto-execute within bounds.
             </p>
          </div>
        </motion.div>
