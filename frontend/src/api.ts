...
const isDashboardMetrics = (value: unknown): value is DashboardMetrics => {
  ...
  if (!operations || !evaluation || !nullableAmount(operations.totalAtRiskPaise) || !nullableAmount(operations.proposalsGenerated) || !nullableAmount(operations.proposalsApproved) || !(operations.attributedRecoveries === null || amount(operations.attributedRecoveries)) || !nullableAmount(operations.recoveredPaise) || !nullableFraction(operations.recoveryRate) || !(operations.contactToRecoveryRatio === null || (typeof operations.contactToRecoveryRatio === 'number' && Number.isFinite(operations.contactToRecoveryRatio) && operations.contactToRecoveryRatio >= 0)) || !['not_run', 'available'].includes(String(evaluation.status)) || !(evaluation.split === null || evaluation.split === 'development' || evaluation.split === 'held_out') || !(evaluation.fixtureSetVersion === null || text(evaluation.fixtureSetVersion)) || !(evaluation.runAt === null || date(evaluation.runAt)) || !(evaluation.configurationHash === null || (typeof evaluation.configurationHash === 'string' && /^[a-f0-9]{64}$/.test(evaluation.configurationHash))) || !(evaluation.modelId === null || text(evaluation.modelId)) || !amount(evaluation.sampleCount) || !nullableFraction(evaluation.precision) || !nullableFraction(evaluation.recall) || !nullableFraction(evaluation.f1) || !nullableAmount(evaluation.falsePositiveCostPaise) || !Array.isArray(row.exceptions) || row.exceptions.length < 6 || row.exceptions.length > 10 || !row.exceptions.every(text)) return false
