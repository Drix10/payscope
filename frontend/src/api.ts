...
const isProposal = (value: unknown): boolean => { const row = object(value); return Boolean(row && text(row.id) && text(row.actionType) && text(row.status) && date(row.proposedAt) && object(row.content)) }
