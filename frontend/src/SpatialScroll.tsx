...
export function SpatialScroll() {
  ...
  useEffect(() => {
    return () => {
      ...
      if (mobileScrollFrame.current !== null) cancelAnimationFrame(mobileScrollFrame.current)
      animationControls.current.forEach((control) => control.stop())
    }
  }, [])
        ...
        <div
          ...
          onScroll={() => {
            ...
            mobileScrollFrame.current = requestAnimationFrame(() => {
              ...
              if (!container) return
              const nextIndex = Math.min(3, Math.max(0, Math.round(container.scrollTop / Math.max(1, container.clientHeight))))
              if (nextIndex !== sectionRef.current) {
